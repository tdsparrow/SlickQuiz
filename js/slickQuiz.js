...
(function($){
  $.slickQuiz = function(element, options) {
    ...
    _quizAssess             = _element + ' .quizAssess',
    ...
    plugin.method = {
      ...
      setupQuiz: function(options) { // use 'options' object to pass args
        ...
        for (i in questions) {
          if (questions.hasOwnProperty(i)) {
            ...
            multi_select     = question.multi_select ? question.multi_select : false,
            forceCheckbox = question.force_checkbox ? question.force_checkbox : false,
            checkbox      = ((multi_select || truths > 1) && !selectAny) || forceCheckbox,
      ...
      resetQuiz: function(startButton, options) {
        ...
        $quizResults.fadeOut(300, function() {
          ...
          $quizAssess.attr('class', 'quizAssess');
      ...
      countScore: function(checkButton, options) {
        var key, keyNotch, kN;
        ...
        answerSelects.each( function() {
          var id = $(this).attr('id');
          var answer = answers[parseInt(id.replace(/(.*\_question\d{1,}_)/, ''), 10)];
      ...
      backToQuestion: function(backButton, options) {
        ...
        if (responses.css('display') === 'block' ) {
          ...
          // clear assessScore
          if (plugin.config.assessmentTest) {
            var answerLIs     = prevQuestion.find(_answers + ' li'),
            answerSelects = answerLIs.find('input:checked'),
            questionIndex = parseInt(questionLI.attr('id').replace(/(question)/, ''), 10),
            answers       = questions[questionIndex].a,
            scores = [];
      ...
      completeQuiz: function(options) {
        ...
        plugin.method.displayScore(score);

        if (plugin.config.disableRanking) {
          $(_quizLevel).remove()
        } else {
          if (!plugin.config.assessmentTest) {
            plugin.method.displayRank(score);
          } else {
            plugin.method.displayAssessment();
          }
      ...
      displayScore: function (score) {
        ...
        if (plugin.config.scoreAsPercentage) {
          display = (score / questionCount).toFixed(2)*100 + "%";
        }
        ...
        if (plugin.config.disableScore) {
          ...
          $(_quizScore + ' span').html(plugin.config.scoreTemplateText
                                       .replace('%score', display).replace('%total', questionCount));
        }
      ...
      displayAssessment: function() {
        var assessments = quizValues.info.assessments;
        $.each(assessments, function(index, value) {
          range = value.range;
          if (assessScore >= range.from && assessScore <= range.to) {
            $(_quizAssess + ' span').html(value.assessment);
          }
        })
    ...
    plugin.init = function() {
      ...
      $(_element + ' ' + _checkAnswerBtn).on('click', function(e) {
        ...
        if (!plugin.config.assessmentTest) {
          ...
          plugin.method.countScore(this, {callback: plugin.config.animationCallbacks.countScore});
        }
